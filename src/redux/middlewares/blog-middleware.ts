...
export const blogMiddleware = (state: MiddlewareAPI) => {
    return (next: Dispatch) => async (action: PayloadAction<any>) => {
        ...
        if (action.type === 'blogs/getBlogs') {
            ...
            try {
                const blogs = await getterBlogFromDB(action.payload, action.payload.blogType)
        ...
        if (action.type === 'blogs/newBlog') {
            try {
                await setBlogInDB(action.payload.entity, action.payload.blog, action.payload.id, action.payload.blogType)
                history.back()
            } catch (err) {
                Swal.fire('Error', 'Ocurrió un Error!', 'error');
            }
        ...
        if (action.type === 'blogs/deleteBlog') {
            ...
            try {
                await deleteBlogInDB(action.payload.entity, action.payload.id, action.payload.blogType)
        ...
        if (action.type === 'blogs/updateBlog') {
            ...
            try {
                await updateBlogsInDB(action.payload.entity, action.payload.blog, action.payload.id, action.payload.blogType)
                Swal.fire({
